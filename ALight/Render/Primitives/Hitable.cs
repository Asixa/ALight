...
namespace ALight.Render.Components
{
    ...
    public class BVHNode : Hitable
    {
        ...
        public BVHNode(Hitable[] p, int n, float time0, float time1)
        {
            ...
            Hitable[] splt(Hitable[] list,int x)
            {
                var result=new Hitable[list.Length-x];
                for (var i = x; i < list.Length; i++) result[i - x] = list[i];
                return result;
            }
            p.ToList().Sort((a, b) => Compare(a, b, (int) (3 * Mathematics.Random.Get())));
            ...
            switch (n)
            {
                ...
                case 2:
                    ...
                    break;
                default:
                    left = new BVHNode(p, n / 2, time0, time1);
                    right = new BVHNode(splt(p,n/2), n - n / 2, time0, time1);
                    break;
            }
    ...
    public class RotateY : Hitable
    {
        ...
        public RotateY(Hitable p, float angle)
        {
            ...
            Console.WriteLine(radians+" " +hasbox+" "+bbox.min+" "+bbox.max);
